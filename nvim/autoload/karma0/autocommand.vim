...
func! karma0#autocommand#apply() abort
  augroup karma0
    au!

    " Set things up, regardless of when we started.
    if v:vim_did_enter
      call s:karma0_neovim_setup()
    else
      au VimEnter * call s:karma0_neovim_setup()
    endif
    ...
    au User vim-pyenv-activate-post call karma0#plugins#update_python()
...
func! s:karma0_neovim_setup() abort
  ...
  if !argc()
    call karma0#projects#show_initial_view()
  endif
endfunc
